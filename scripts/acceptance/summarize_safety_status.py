#!/usr/bin/env python3
"""Summarize cargo diagnostics and validate observable bag contracts."""
...
def validate_avoidance_rows(
    ...
    """Check only contracts observable in a legacy cargo CSV.

    Far-history maturity itself remains a tracker/GTest and runtime diagnostic
    contract. A bag that never contains the prerequisite trajectory is
    NOT_COVERED, not a failure; --require-code is an explicit coverage gate.
    """
    violations: list[str] = []
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=[],
        help="require a warning code to occur (repeatable coverage gate)",
    )
    ...
    query_reasons: collections.Counter[str] = collections.Counter()
    ...
    summary = {
        "rows": len(cargo_rows),
        "requested_codes": dict(codes),
        "safety_reasons": dict(reasons),
        "static_query_reasons": dict(query_reasons),
        "obstacle_track_ids": sorted(track_ids),
        "last_static_index_cells": last.get("static_index_cell_count", "0"),
        "last_track_created_count": last.get(
            "obstacle_track_created_count", "0"),
        "last_track_reset_count": last.get(
            "obstacle_track_reset_count", "0"),
