...
CargoPhysicalIdentityDecision CargoPhysicalIdentityAuthority::update(
  ...
  for (std::size_t gi = 0; gi < input.groups.size(); ++gi) {
    ...
    const bool strong_descriptor = finiteDescriptor(group.descriptor);
    // During PRELOAD (EMPTY) a weak descriptor is skipped to preserve the
    // prelift reference-freeze timing; during POSTLOAD it may RECOVERY_HOLD
    // below instead of fragmenting the history.
    if (!strong_descriptor && !gravity_loaded) continue;
    ...
    if (group_match[gi] >= 0) {
      ...
      if (gravity_loaded) {
      // POSTLOAD RECOVERY_HOLD: a single recent, spatially-overlapping history
      // may be held alive through an INVALID / XY-extent-churn gap WITHOUT
      ...
      if (!strong_descriptor) {
        // POSTLOAD weak descriptor with no recovery history: don't fragment.
    ...
    // geometry-confirming) evidence; this is the timestamp POSTLOAD
    // RECOVERY_HOLD keys its bounded coast window from.  Held frames do NOT
    // advance it.
