...
void RuntimeDiagnostics::configure(const RuntimeDiagnosticsConfig& cfg,
  ...
  if (cfg_.enabled && cfg_.csv_enabled) {
    ...
    ndt_csv_ << "frame_index,cloud_stamp,sensor_dt_ms,wall_interarrival_ms,"
             ...
             << "local_map_pose_authority,local_map_consecutive_prediction_only_frames,"
             << "local_map_prediction_only_duration_sec,local_map_frames_since_last_trusted_ndt,"
             << "local_map_time_since_last_trusted_ndt_sec,"
             << "local_map_distance_since_last_trusted_ndt_m,"
             << "local_map_yaw_since_last_trusted_ndt_rad,local_map_update_eligible,"
             << "local_map_update_due,local_map_update_committed,local_map_update_mode,"
             << "local_map_health_state,local_map_version,local_map_point_count,"
             << "local_map_updates_from_measured_pose,local_map_updates_from_predicted_pose,"
             << "local_map_motion_escape_refresh_count,"
             << "motion_gate_stationary,stationary_entry_confirm_count,"
             << "stationary_entry_unreliable_count,motion_gate_velocity_modified,"
    ...
    cargo_csv_ << "stamp,track_state,track_id,lock_state,observation_valid,"
               ...
               << "candidate_present,candidate_authoritative,candidate_points,"
               << "candidate_age_sec,candidate_consistent_frames,"
               << "candidate_required_frames,candidate_merged_components,"
               << "candidate_long_axis_coverage,candidate_short_axis_coverage,"
               ...
               << "requested_alarm_code,safety_evidence_state,raw_warning_code,"
               ...
               << "obstacle_track_static,obstacle_static_provenance_streak,"
               << "obstacle_static_age_sec,obstacle_large_geometry_valid,"
               << "obstacle_provenance_type,obstacle_provenance_valid,"
               ...
               << "height_evidence_age_sec,vertical_evidence_age_sec,"
               << "top_support_points,top_surface_coverage,"
               << "locked_obb_support_ratio,vertical_reject_reason,"
               << "association_reject_reason,"
...
void RuntimeDiagnostics::writeNdtFrame(const NdtFrameRecord& rec) {
  ...
  if (cfg_.csv_enabled && ndt_csv_.is_open()) {
    ndt_csv_ << rec.frame_index << ","
             ...
             << rec.local_map_pose_authority << ","
             << rec.local_map_consecutive_prediction_only_frames << ","
             << rec.local_map_prediction_only_duration_sec << ","
             << rec.local_map_frames_since_last_trusted_ndt << ","
             << rec.local_map_time_since_last_trusted_ndt_sec << ","
             << rec.local_map_distance_since_last_trusted_ndt_m << ","
             << rec.local_map_yaw_since_last_trusted_ndt_rad << ","
             << (rec.local_map_update_eligible ? 1 : 0) << ","
             << (rec.local_map_update_due ? 1 : 0) << ","
             << (rec.local_map_update_committed ? 1 : 0) << ","
             << rec.local_map_update_mode << ","
             << rec.local_map_health_state << ","
             << rec.local_map_version << ","
             << rec.local_map_point_count << ","
             << rec.local_map_updates_from_measured_pose << ","
             << rec.local_map_updates_from_predicted_pose << ","
             << rec.local_map_motion_escape_refresh_count << ","
             << (rec.motion_gate_stationary ? 1 : 0) << ","
             << rec.stationary_entry_confirm_count << ","
             << rec.stationary_entry_unreliable_count << ","
...
void RuntimeDiagnostics::writeCargoFrame(const CargoFrameRecord& rec) {
  ...
  if (cargo_csv_.is_open()) {
    cargo_csv_ << std::fixed << std::setprecision(6) << rec.stamp << ","
               ...
               << (rec.candidate_present ? 1 : 0) << ","
               << (rec.candidate_authoritative ? 1 : 0) << ","
               << rec.candidate_points << "," << rec.candidate_age_sec << ","
               << rec.candidate_consistent_frames << ","
               << rec.candidate_required_frames << ","
               << rec.candidate_merged_components << ","
               << rec.candidate_long_axis_coverage << ","
               << rec.candidate_short_axis_coverage << ","
               << rec.candidate_gate_failure << ","
               ...
               << rec.obstacle_static_age_sec << ","
               << (rec.obstacle_large_geometry_valid ? 1 : 0) << ","
               << rec.obstacle_provenance_type << ","
               << (rec.obstacle_provenance_valid ? 1 : 0) << ","
