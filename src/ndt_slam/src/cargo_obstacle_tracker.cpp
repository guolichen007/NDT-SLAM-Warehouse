  ...
  for (CargoObstacleObservation observation : observations) {
    ...
    if (!track.far_field_history_valid) {
      if (qualifiesForFarHistory(observation, config_)) {
        ...
        // Entry into <=5 m before both evidence gates complete remains an
        // anomalous near-field appearance (code 29); do not promote it merely
        // because one earlier sample happened to be just above 5 m.
        track.far_field_validated_observations = 0;
