...
static_assert(
    static_cast<std::uint8_t>(
        ...
        lidar_slam2_msgs::CargoBottomEstimate::
            SOURCE_DIRECT_TOP_FROZEN_THICKNESS,
    "Cargo source schema drift: DIRECT_TOP_FROZEN_THICKNESS");
...
NdtSlamNode::NdtSlamNode(const ros::NodeHandle& nh)
    ...
    cargo_pending_avoidance_pub_ = nh_.advertise<std_msgs::String>(
        "/cargo_avoidance/pending_status", 1);
    cargo_pending_envelope_marker_pub_ =
        nh_.advertise<visualization_msgs::Marker>(
            "/cargo_avoidance/pending_envelope_marker", 1);
    static_evidence_status_pub_ = nh_.advertise<std_msgs::String>(
        "/static_evidence/status", 1, true);
    static_evidence_cell_state_counts_pub_ = nh_.advertise<std_msgs::String>(
        "/static_evidence/cell_state_counts", 1, true);
    static_evidence_streak_histogram_pub_ = nh_.advertise<std_msgs::String>(
        "/static_evidence/streak_histogram", 1, true);
    ...
    load_map_session_srv_ = nh_.advertiseService(
        "load_map_session", &NdtSlamNode::loadMapSessionService, this);
...
void NdtSlamNode::initializeParameters(const std::string& config_file_path) {
    try {
        ...
        if (config["hook_cargo_lock"]) {
            ...
            hook_lock_config_.formal_top_min_support_points = std::max(
                1, hcl["formal_top_min_support_points"].as<int>(8));
            hook_lock_config_.formal_top_min_coverage_ratio = std::clamp(
                hcl["formal_top_min_coverage_ratio"].as<float>(0.02F),
                0.0F, 1.0F);
            ...
            hook_lock_config_.loaded_reacquire_min_bottom_uncertainty_m =
                std::max(0.0F,
                    hcl["loaded_reacquire_min_bottom_uncertainty_m"]
                        .as<float>(0.12F));
        ...
        if (config["cargo_lift_origin"]) {
            ...
            cargo_lift_origin_config_.maximum_anchor_distance_m =
                lift["maximum_anchor_distance_m"].as<float>(2.0F);
            cargo_lift_origin_config_.minimum_significant_change_m =
                lift["minimum_significant_change_m"].as<float>(0.15F);
            cargo_lift_origin_config_.significance_sigma =
                lift["significance_sigma"].as<float>(3.0F);
            cargo_lift_origin_config_.minimum_source_coverage =
                lift["minimum_source_coverage"].as<float>(0.35F);
            cargo_lift_origin_config_.minimum_revealed_support_coverage =
                lift["minimum_revealed_support_coverage"].as<float>(0.25F);
            cargo_lift_origin_config_.lift_confirm_frames =
                lift["lift_confirm_frames"].as<int>(4);
            cargo_lift_origin_config_.thickness_confirm_frames =
                lift["thickness_confirm_frames"].as<int>(5);
            cargo_lift_origin_config_.maximum_observation_gap_sec =
                lift["maximum_observation_gap_sec"].as<double>(0.5);
            cargo_lift_origin_config_.maximum_source_age_sec =
                lift["maximum_source_age_sec"].as<double>(0.5);
            cargo_lift_origin_config_.minimum_height_m =
                lift["minimum_height_m"].as<float>(0.30F);
            cargo_lift_origin_config_.maximum_height_m =
                lift["maximum_height_m"].as<float>(5.00F);
        }
        cargo_lift_origin_binder_.setConfig(cargo_lift_origin_config_);
        ...
        if (config["cargo_geometry_fusion"]) {
            ...
            cargo_geometry_fusion_config_.maximum_source_disagreement_m =
                geometry["maximum_source_disagreement_m"].as<float>(0.25F);
            cargo_geometry_fusion_config_.maximum_fused_uncertainty_m =
        ...
        if (config["pending_cargo_envelope"]) {
            ...
            pending_cargo_envelope_config_.configured_length_m =
                pending["configured_length_m"].as<float>(4.0F);
            ...
            pending_cargo_envelope_config_.vertical_margin_m =
                pending["vertical_margin_m"].as<float>(0.15F);
        ...
        if (cargo_safety) {
            ...
            static_height_field_config_.z_bin_m = std::max(
                0.02F, cargo_safety["static_height_z_bin_m"]
                           .as<float>(0.10F));
            static_height_field_config_.maximum_layers_per_cell =
                static_cast<std::size_t>(std::clamp(
                    cargo_safety["static_height_max_layers_per_cell"]
                        .as<int>(3), 1, 8));
            static_height_field_config_.minimum_points_per_layer =
                static_cast<std::size_t>(std::max(
                    1, cargo_safety["static_height_min_points_per_layer"]
                           .as<int>(6)));
            static_height_field_config_.maximum_merge_gap_m = std::max(
                static_height_field_config_.z_bin_m,
                cargo_safety["static_height_maximum_merge_gap_m"]
                    .as<float>(0.18F));
            static_height_field_config_.maximum_query_area_cells =
                static_map_config.maximum_query_area_cells;
            cargo_avoidance_fusion_config_
                .minimum_live_coverage_for_clear = std::clamp(
...
void NdtSlamNode::resetCargoForHookState(
    bool preserve_origin_height, bool preserve_retired_signature) {
    clearHookLock();
    ...
    cargo_hook_state_initialized_ = false;
...
void NdtSlamNode::startCleanMapRebuildJob() {
    ...
    const std::uint64_t source_static_evidence_epoch =
        static_evidence_epoch_.load(std::memory_order_acquire);
    const auto source_static_evidence_snapshot =
        static_obstacle_evidence_index_.snapshot();
    const StaticEvidenceAuthority source_static_authority =
        source_static_evidence_snapshot
            ? source_static_evidence_snapshot->authority
            : StaticEvidenceAuthority::RUNTIME_MATURE;
    MapLayerBundle source_bundle;
    StaticEvidenceCellGeometryMap source_observed_cells;
    StaticEvidenceCellKeySet source_observable_cells;
    StaticEvidenceCellKeySet source_free_cells;
    {
        std::lock_guard<std::mutex> observation_lock(
            static_observation_mutex_);
        source_observed_cells.swap(pending_static_observed_cells_);
        source_observable_cells.swap(pending_static_observable_cells_);
        source_free_cells.swap(pending_static_free_cells_);
    }
    {
        std::lock_guard<std::mutex> lock(map_mutex_);
        const auto clone = [](const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
    ...
    clean_map_rebuild_thread_ = std::thread(
        [this, source_objects_version, source_static_evidence_epoch,
         ...
         source_observed_cells = std::move(source_observed_cells),
         source_observable_cells = std::move(source_observable_cells),
         source_free_cells = std::move(source_free_cells),
         ...
         input = std::move(input)]() mutable {
            ...
            result.static_observed_cells =
                std::move(source_observed_cells);
            ...
            try {
                ...
                if (result.valid) {
                    ...
                    if (result.bundle.ground) {
                        ground_points.reserve(result.bundle.ground->size());
                        for (const auto& point :
                             result.bundle.ground->points) {
                            ground_points.emplace_back(
                                point.x, point.y, point.z);
                        }
                    }
                    result.static_height_field =
                        std::make_shared<StaticHeightField>(
                            static_height_field_config_);
                    std::vector<Eigen::Vector3f> authorized_objects;
                    if (source_static_authority ==
                        StaticEvidenceAuthority::RUNTIME_MATURE) {
                        authorized_objects.reserve(
                            result.build.clean_points.size());
                        const float cell_size =
                            source_static_evidence_snapshot
                                ? source_static_evidence_snapshot->cell_size_m
                                : 0.25F;
                        for (const Eigen::Vector3f& point :
                             result.build.clean_points) {
                            if (!source_static_evidence_snapshot) continue;
                            const std::int64_t key =
                                packStaticEvidenceCell(
                                    static_cast<std::int32_t>(std::floor(
                                        point.x() / cell_size)),
                                    static_cast<std::int32_t>(std::floor(
                                        point.y() / cell_size)));
                            const auto found =
                                source_static_evidence_snapshot->cells.find(
                                    key);
                            if (found !=
                                    source_static_evidence_snapshot->cells.end() &&
                                found->second.clean_map_confirmed &&
                                found->second.temporally_mature) {
                                authorized_objects.push_back(point);
                            }
                        }
                    } else {
                        authorized_objects = result.build.clean_points;
                    }
                    result.static_height_field->build(
                        authorized_objects, ground_points,
                        source_static_authority);
                }
...
void NdtSlamNode::consumeCleanMapRebuildResult(const ros::Time& stamp) {
    ...
    const auto restore_observations = [this, &result]() {
        std::lock_guard<std::mutex> observation_lock(
            static_observation_mutex_);
        for (const auto& item : result.static_observed_cells) {
            if (pending_static_free_cells_.find(item.first) !=
                pending_static_free_cells_.end()) {
                continue;
            }
            auto inserted = pending_static_observed_cells_.emplace(
                item.first, item.second);
            if (!inserted.second) {
                inserted.first->second.min_z = std::min(
                    inserted.first->second.min_z, item.second.min_z);
                inserted.first->second.max_z = std::max(
                    inserted.first->second.max_z, item.second.max_z);
            }
        }
        pending_static_observable_cells_.insert(
            result.static_observable_cells.begin(),
            result.static_observable_cells.end());
        for (const std::int64_t key : result.static_free_cells) {
            if (pending_static_observed_cells_.find(key) ==
                pending_static_observed_cells_.end()) {
                pending_static_free_cells_.insert(key);
            }
        }
    };
    ...
    if (result.static_evidence_epoch ==
        ...
        status_json << "{\"generation\":"
                    ...
                    << staticEvidenceAuthorityName(
                           evidence_snapshot
                               ...
                               : StaticEvidenceAuthority::
                                     UNVERIFIED_LOADED_CLEAN)
                    << "\",\"clean_observation_sequence\":"
                    << static_obstacle_evidence_index_.
                           latestObservationSequence()
                    << "}";
        status_message.data = status_json.str();
        static_evidence_status_pub_.publish(status_message);

        std_msgs::String counts_message;
        std::ostringstream counts_json;
        counts_json << "{\"working\":" << evidence_status.working_cells
                    << ",\"clean_confirmed\":"
                    << evidence_status.clean_confirmed_cells
                    << ",\"temporally_mature\":"
                    << evidence_status.temporally_mature_cells
                    << ",\"pending_observation_count\":"
                    << evidence_status.pending_observation_count
                    << ",\"pending_stable_duration\":"
                    << evidence_status.pending_stable_duration
                    << ",\"reset_by_time_gap\":"
                    << evidence_status.reset_by_time_gap
                    << ",\"reset_by_sequence_gap\":"
                    << evidence_status.reset_by_sequence_gap
                    << ",\"invalidated_by_tombstone\":"
                    << evidence_status.invalidated_by_tombstone
                    << ",\"generation_mismatch\":"
                    ...
                    << ",\"observed_free\":"
                    << evidence_status.observed_free
                    << ",\"observed_occupied\":"
                    << evidence_status.observed_occupied << "}";
        counts_message.data = counts_json.str();
        static_evidence_cell_state_counts_pub_.publish(counts_message);

        std_msgs::String histogram_message;
        std::ostringstream histogram_json;
        histogram_json << "{";
        bool first_histogram_entry = true;
        for (const auto& item : evidence_status.streak_histogram) {
            if (!first_histogram_entry) histogram_json << ',';
            first_histogram_entry = false;
            histogram_json << '\"' << item.first << "\":" << item.second;
        }
        histogram_json << "}";
        histogram_message.data = histogram_json.str();
        static_evidence_streak_histogram_pub_.publish(histogram_message);
        ...
        if (result.static_height_field) {
            std::atomic_store(
                &static_height_field_,
                std::static_pointer_cast<const StaticHeightField>(
                    result.static_height_field));
        }
    }
...
void NdtSlamNode::processCloudThread() {
    ...
    while (ros::ok() && !shutdown_) {
        ...
        if (hook_load_signal_role_ == HookLoadSignalRole::REQUIRED &&
            ...
            resetCargoForHookState(entering_loaded, true);
...
void NdtSlamNode::updatePoseFromLoopClosure(
    ...
    loaded_map_session_generation_ = 0U;
    {
        std::lock_guard<std::mutex> observation_lock(
            static_observation_mutex_);
        pending_static_observed_cells_.clear();
        pending_static_observable_cells_.clear();
...
bool NdtSlamNode::saveMapService(lidar_slam2_msgs::SaveMap::Request& request,
    ...
    std::size_t registration_points = 0U;
    {
        std::lock_guard<std::mutex> lock(map_mutex_);
        if (latest_completed_map_bundle_.registration) {
            registration_points =
                latest_completed_map_bundle_.registration->size();
        }
    }

    if (registration_points == 0U) {
        response.success = false;
        response.message = "Completed registration map is empty";
    ...
    try {
        ...
        std::filesystem::path parent = requested_path.parent_path();
        if (parent.empty()) parent = ".";
        const std::string session_dir =
            (parent / ("session_" +
                std::to_string(ros::Time::now().toNSec()))).string();

        updateKeyFrameMetrics();
        std::string save_reason;
        response.success = saveMapSessionAtomic(session_dir, &save_reason);
        response.num_points = static_cast<std::int64_t>(registration_points);
        response.saved_file_path = response.success
            ? (std::filesystem::path(session_dir) / "manifest.yaml").string()
            : "";
        if (!response.success) {
            response.message = "Atomic map session save failed: " +
                save_reason;
            ROS_ERROR("[SaveMap] atomic session failed: %s",
                      save_reason.c_str());
            return true;
        }

        // Retain the legacy single-PCD artifact only as a compatibility copy.
        // It is copied after the complete session has been atomically
        // published and is never used as proof that the transaction succeeded.
        std::error_code copy_error;
        std::filesystem::copy_file(
            std::filesystem::path(session_dir) / "map_registration.pcd",
            requested_path, std::filesystem::copy_options::overwrite_existing,
            copy_error);
        response.message = copy_error
            ? "Atomic map session saved; legacy PCD copy failed: " +
                  copy_error.message()
            : "Atomic map session and legacy registration PCD saved";
        ROS_INFO("[SaveMap] coherent session saved: %s, points=%zu",
                 session_dir.c_str(), registration_points);
    } catch (const std::exception& e) {
        response.success = false;
...
bool NdtSlamNode::installLoadedRuntimeMap(
    ...
    try {
        ...
        response.message = relocalization_enabled_
            ? (session_mode
                   ? "Verified map session loaded; global relocalization required"
                   : "Map loaded; global relocalization required")
            : (session_mode
                   ? "Verified map session loaded; runtime state reset"
...
bool NdtSlamNode::rebuildMapService(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response) {
    ...
    if (!saveMapSessionAtomic(session_dir, &session_reason)) {
        ROS_ERROR("Atomic rebuilt-map session save failed: %s",
                  session_reason.c_str());
        return false;
    }
...
bool NdtSlamNode::saveMapSessionAtomic(const std::string& session_dir,
                                       std::string* reason) {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr ground_raw_snapshot;
    {
        ...
        request.metadata.objects_content_version =
            latest_completed_map_bundle_.objects_version;
        request.metadata.clean_build_version =
    ...
    request.write_extras =
        [this, filtered_snapshot, payload_candidate_snapshot,
         ...
         ground_raw_snapshot](const std::string& temporary,
            ...
            const auto save = [&diagnostics](
                    const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,
                    const char* filename) {
                return !cloud || cloud->empty() ||
                    pcl::io::savePCDFileBinary(
                        (diagnostics / filename).string(), *cloud) >= 0;
            };
            if (!save(filtered_snapshot, "map_objects_filtered.pcd") ||
                !save(payload_candidate_snapshot,
                      "map_payload_candidate.pcd") ||
                !save(payload_dynamic_snapshot,
                      "map_payload_dynamic.pcd") ||
                !save(human_candidate_snapshot,
                      "map_human_candidate.pcd") ||
                !save(human_dynamic_snapshot, "map_human_dynamic.pcd") ||
                !save(human_pending_snapshot, "map_human_pending.pcd") ||
                !save(ground_raw_snapshot, "map_ground_raw.pcd")) {
                if (extras_reason) *extras_reason = "diagnostic_pcd";
                return false;
            }
            const auto diagnostics_state =
                static_obstacle_evidence_index_.diagnostics();
            std::ofstream runtime_status(
                std::filesystem::path(temporary) /
                "runtime_status_at_save.json");
            runtime_status
                << "{\n  \"static_working_cells\": "
                << diagnostics_state.working_cells
                << ",\n  \"static_clean_confirmed_cells\": "
                << diagnostics_state.clean_confirmed_cells
                << ",\n  \"static_temporally_mature_cells\": "
                << diagnostics_state.temporally_mature_cells
                << "\n}\n";
            runtime_status.flush();
            if (!runtime_status.good()) {
                if (extras_reason) *extras_reason = "runtime_status";
                return false;
            }
...
void NdtSlamNode::updateLiveCargoPose(
    ...
    const bool supported_top_surface =
        hook_lock_config_.track_vertical_from_top_surface &&
        std::isfinite(det.z95) &&
        det.top_support_points >= static_cast<std::size_t>(
            hook_lock_config_.formal_top_min_support_points) &&
        det.top_surface_coverage_ratio >=
...
void NdtSlamNode::updateHookCargoLock(
    ...
    case HookCargoLockState::CLEAR_WAIT_REARM: {
        const bool loaded_reacquire_candidate =
            hook.valid && gravity_state == HookLoadState::LOADED &&
            retired_cargo_signature_valid_ && retired_cargo_shape_.valid &&
            candidate_policy.allow_candidate && det.valid && bottom.valid &&
            det.oriented_footprint_valid &&
            det.identity_confidence >=
                hook_lock_config_.minimum_identity_confidence &&
            (det.center_base.head<2>() -
             retired_cargo_center_base_.head<2>()).norm() <=
                hook_lock_config_.reacquisition_max_xy_gate_m;
        if (loaded_reacquire_candidate) {
            ...
            hook_lock_.has_last_accepted = true;
            ROS_WARN(
                "[CARGO_STATE] CLEAR_WAIT_REARM->LOADED_REACQUIRE "
                "identity=%.2f center_gate=%.2f",
                det.identity_confidence,
                (det.center_base.head<2>() -
                 retired_cargo_center_base_.head<2>()).norm());
            break;
    ...
    case HookCargoLockState::LOADED_REACQUIRE: {
        if (!hook.valid || gravity_state != HookLoadState::LOADED ||
            !retired_cargo_signature_valid_ ||
            !retired_cargo_shape_.valid) {
            hook_lock_.state = HookCargoLockState::CLEAR_WAIT_REARM;
            hook_lock_.confirm_count = 0;
            hook_lock_.rearm_start_stamp = stamp;
            ROS_WARN("[CargoLock] LOADED_REACQUIRE->CLEAR_WAIT_REARM "
                     "reason=loaded_or_signature_invalid");
            break;
        }
        ...
        const float length_error = std::abs(
            det.footprint_length_width.x() -
            retired_cargo_shape_.length_m) /
            std::max(0.10F, retired_cargo_shape_.length_m);
        const float width_error = std::abs(
            det.footprint_length_width.y() -
            retired_cargo_shape_.width_m) /
            std::max(0.10F, retired_cargo_shape_.width_m);
        const bool consistent = det.valid && bottom.valid &&
            det.oriented_footprint_valid &&
            det.identity_confidence >=
                hook_lock_config_.minimum_identity_confidence &&
            center_error <=
                hook_lock_config_.reacquisition_max_xy_gate_m &&
            std::max(length_error, width_error) <=
        ...
        if (!consistent) {
            ...
            if (hook_lock_.weak_count >
                ...
                hook_lock_.rearm_start_stamp = stamp;
                ROS_WARN("[CargoLock] LOADED_REACQUIRE->CLEAR_WAIT_REARM "
                         "reason=identity_or_geometry_inconsistent");
            }
            break;
        }
        hook_lock_.weak_count = 0;
        ...
        hook_lock_.locked_shape = retired_cargo_shape_;
        hook_lock_.locked_shape.valid = true;
        hook_lock_.locked_size = Eigen::Vector3f(
            retired_cargo_shape_.length_m,
            retired_cargo_shape_.width_m,
            retired_cargo_shape_.height_m);
        hook_lock_.has_locked_size = true;
        hook_lock_.stable_height = retired_cargo_shape_.height_m;
        hook_lock_.stable_top_z = bottom.top_z_base;
        hook_lock_.stable_bottom_z =
            bottom.top_z_base - retired_cargo_shape_.height_m;
        hook_lock_.bottom_uncertainty = std::max(
            bottom.uncertainty,
            hook_lock_config_.loaded_reacquire_min_bottom_uncertainty_m);
        hook_lock_.has_good_height = true;
        hook_lock_.shape_height_valid = true;
        hook_lock_.last_accepted_core_points = det.core_points_base;
        hook_lock_.has_last_accepted = true;
        hook_lock_.state = HookCargoLockState::LOCKED;
        updateLiveCargoPose(
        ...
        ROS_WARN(
            "[CARGO_STATE] LOADED_REACQUIRE->LOCKED track_segment=%llu "
            "identity=%.2f center_error=%.2f",
            static_cast<unsigned long long>(cargo_fusion_track_id_),
            det.identity_confidence, center_error);
        break;
    }
    }
...
void NdtSlamNode::publishCargoFusionMarker(
    ...
    if (lifecycle.style == CargoMarkerStyle::LOCALIZATION_DEGRADED) {
        ...
        // Green means formal geometry + vertical authority are valid. Source
        // provenance belongs in CargoBottomEstimate/CSV, not marker colour.
        marker.color.r = 0.1F;
        marker.color.g = 1.0F;
        marker.color.b = 0.1F;
    }
...
lidar_slam2_msgs::CargoSafetyStatus NdtSlamNode::composeCargoSafetyStatus(
    ...
    static_assert(
        Status::PROVENANCE_NONE ==
                static_cast<std::uint8_t>(ExternalProvenance::NONE) &&
            Status::PROVENANCE_OUTSIDE_CARGO_SHELL_ONLY ==
                static_cast<std::uint8_t>(
                    ExternalProvenance::OUTSIDE_CARGO_SHELL_ONLY) &&
            Status::PROVENANCE_PRE_CARGO_OCCUPANCY ==
                static_cast<std::uint8_t>(
                    ExternalProvenance::PRE_CARGO_OCCUPANCY) &&
            Status::PROVENANCE_STATIC_MAP_MATCH ==
                static_cast<std::uint8_t>(
                    ExternalProvenance::STATIC_MAP_MATCH) &&
            Status::PROVENANCE_CARGO_MOVED_AWAY_PERSISTENCE ==
                static_cast<std::uint8_t>(
                    ExternalProvenance::CARGO_MOVED_AWAY_PERSISTENCE) &&
            Status::PROVENANCE_DUAL_LIDAR_CONSENSUS ==
                static_cast<std::uint8_t>(
    ...
    if (status.requested_alarm_code >= CargoSafetyProtocol::kSystemNotReady &&
        status.requested_alarm_code != CargoSafetyProtocol::kObstacleInvalid) {
...
void NdtSlamNode::logCargoSafetyStatus(
    ...
    // cargo_alarm_heartbeat is the sole operator-console owner for 14/17/18
    // and 30--34. The producer retains full per-frame CSV/status fields and
    // prints only unrecoverable internal contract failures.
    if (code_changed && status.requested_alarm_code ==
                            CargoSafetyProtocol::kInternalError) {
        ROS_ERROR_STREAM(
            "[CARGO_SAFETY_INTERNAL] code=35 reason=" << status.reason);
    }
}
...
void NdtSlamNode::publishHookOnlySafetyStatus(
    ...
    bool evidence_initialized, std::int32_t provisional_warning_code,
    std::uint32_t provisional_obstacle_count,
...
void NdtSlamNode::updateCargoLiftAndGeometryFusion(
    ...
    const bool hook_was_empty = hook_loaded &&
        cargo_hook_state_initialized_ && !cargo_previous_hook_loaded_;
    if (hook_loaded && (node_started_loaded || hook_was_empty)) {
        ++cargo_lifecycle_id_;
        if (cargo_lifecycle_id_ == 0U) ++cargo_lifecycle_id_;
        cargo_lift_origin_binder_.reset();
        cargo_geometry_fusion_.reset();
        cargo_frozen_geometry_ = CargoFrozenGeometry{};
    }
    if (active_track && !cargo_fusion_track_active_) {
        ...
        if (cargo_track_segment_id_ == 0U) ++cargo_track_segment_id_;
    }
    cargo_hook_state_initialized_ = hook.valid;
    ...
    const StaticEvidenceAuthority snapshot_authority = evidence_snapshot
        ? evidence_snapshot->authority
        : StaticEvidenceAuthority::UNVERIFIED_LOADED_CLEAN;

    CargoLiftOriginInput lift_input;
    lift_input.stamp_sec = stamp.toSec();
    lift_input.hook_signal_valid = hook.valid;
    lift_input.hook_loaded = hook_loaded;
    lift_input.hook_was_empty = hook_was_empty;
    lift_input.node_started_loaded = node_started_loaded;
    lift_input.anchor_valid = anchor_map.allFinite();
    lift_input.hook_anchor_map = anchor_map;

    const bool detection_current = hook_fixed_cargo_.valid &&
    ...
    if (detection_current && std::isfinite(hook_fixed_cargo_.z95)) {
        const Eigen::Vector3d top_map = pose_map_base * Eigen::Vector3d(
            hook_fixed_cargo_.center_base.x(),
            hook_fixed_cargo_.center_base.y(),
            hook_fixed_cargo_.z95);
        lift_input.current_top_valid = std::isfinite(top_map.z());
        lift_input.current_top_stamp_sec = stamp.toSec();
        lift_input.current_top_z_map = static_cast<float>(top_map.z());
        lift_input.current_top_uncertainty_m =
            std::max(0.02F, hook_fixed_bottom_.uncertainty);
        lift_input.source_coverage = std::clamp(std::max({
            hook_fixed_cargo_.long_axis_coverage_ratio,
            hook_fixed_cargo_.short_axis_coverage_ratio,
            hook_fixed_cargo_.top_surface_coverage_ratio}), 0.0F, 1.0F);
    }
    if (detection_current && hook_fixed_cargo_.ground_reference_valid &&
        std::isfinite(hook_fixed_cargo_.ground_z)) {
        const Eigen::Vector3d support_map = pose_map_base * Eigen::Vector3d(
            anchor_base.x(), anchor_base.y(), hook_fixed_cargo_.ground_z);
        lift_input.revealed_support_valid = std::isfinite(support_map.z());
        lift_input.revealed_support_stamp_sec = stamp.toSec();
        lift_input.revealed_support_z_map =
            static_cast<float>(support_map.z());
        lift_input.revealed_support_coverage =
            lift_input.source_coverage;
    }

    if (height_field && anchor_map.allFinite()) {
        StaticHeightQuery origin_query;
        origin_query.center_map = anchor_map;
        origin_query.length_m =
            pending_cargo_envelope_config_.configured_length_m;
        origin_query.width_m =
            pending_cargo_envelope_config_.configured_width_m;
        origin_query.shell_m = 0.0F;
        const StaticHeightQueryResult origin_height =
            height_field->query(origin_query);
        const StaticSupportSurfaceCell support =
            height_field->supportAt(anchor_map);
        if (origin_height.valid && origin_height.matched_cells > 0U &&
            std::isfinite(origin_height.highest_z95_m) && support.valid &&
            std::isfinite(support.z)) {
            CargoOriginCandidate candidate;
            candidate.component_id = origin_height.matched_cell_keys.empty()
                ? 1U
                : static_cast<std::uint64_t>(
                      origin_height.matched_cell_keys.front());
            candidate.source = snapshot_authority ==
            ...
            candidate.authority = snapshot_authority;
            candidate.center_map = anchor_map;
            candidate.length_m = origin_query.length_m;
            candidate.width_m = origin_query.width_m;
            candidate.top_z95_map = origin_height.highest_z95_m;
            candidate.support_z_map = support.z;
            candidate.uncertainty_m = std::max(
                origin_height.highest_uncertainty_m,
                support.uncertainty_m);
            candidate.occupied_cells = origin_height.matched_cells;
    ...
    if (retired_cargo_signature_valid_ && retired_cargo_shape_.valid &&
        ...
        if (age_sec >= 0.0 && age_sec <=
            ...
            CargoOriginCandidate candidate;
            candidate.component_id = cargo_lifecycle_id_;
            candidate.source =
                CargoOriginCandidateSource::RETIRED_FORMAL_SHAPE;
            candidate.authority = snapshot_authority;
            candidate.center_map =
                retired_center_map.head<2>().cast<float>();
            candidate.length_m = retired_cargo_shape_.length_m;
            candidate.width_m = retired_cargo_shape_.width_m;
            candidate.yaw_map_rad = retired_cargo_shape_.yaw_base_rad;
            candidate.top_z95_map = static_cast<float>(retired_center_map.z()) +
                0.5F * retired_cargo_shape_.height_m;
            candidate.support_z_map = candidate.top_z95_map -
                retired_cargo_shape_.height_m;
            candidate.uncertainty_m = 0.15F;
            lift_input.candidates.push_back(candidate);
        }
    }

    // Configured origin is only a binding/pending fallback. It is never
    // counted as an independent static thickness source below.
    CargoOriginCandidate configured_origin;
    configured_origin.component_id = 1U;
    configured_origin.source =
        CargoOriginCandidateSource::CONFIGURED_ENVELOPE;
    configured_origin.center_map = anchor_map;
    configured_origin.length_m =
        pending_cargo_envelope_config_.configured_length_m;
    configured_origin.width_m =
        pending_cargo_envelope_config_.configured_width_m;
    configured_origin.top_z95_map = static_cast<float>(anchor_map_3d.z()) +
        hook_fixed_config_.roi_z_max;
    configured_origin.support_z_map = configured_origin.top_z95_map -
        pending_cargo_envelope_config_.configured_height_m;
    configured_origin.uncertainty_m =
        pending_cargo_envelope_config_.vertical_margin_m;
    lift_input.candidates.push_back(configured_origin);
    if (cargo_lift_origin_enabled_) {
        cargo_lift_origin_result_ =
            cargo_lift_origin_binder_.update(lift_input);
    } else {
        cargo_lift_origin_result_ = CargoLiftOriginResult{};
    ...
    if (cargo_lift_origin_result_.valid) {
        ...
        candidate.yaw_base_rad = origin.yaw_map_rad;
    ...
    if (!hook_loaded || cargo_lifecycle_id_ == 0U) return;
    CargoGeometryFrame geometry_frame;
    geometry_frame.cargo_lifecycle_id = cargo_lifecycle_id_;
    geometry_frame.track_segment_id = cargo_track_segment_id_;
    geometry_frame.stamp_sec = stamp.toSec();
    ...
    if (authorized_origin && cargo_lift_origin_result_.lift_confirmed &&
    ...
    if (active_track && cargo_frozen_geometry_.valid &&
        ...
        hook_lock_.locked_shape.valid = true;
        hook_lock_.locked_shape.length_m = cargo_frozen_geometry_.length_m;
        hook_lock_.locked_shape.width_m = cargo_frozen_geometry_.width_m;
        hook_lock_.locked_shape.height_m = cargo_frozen_geometry_.height_m;
        hook_lock_.locked_shape.yaw_base_rad = cargo_frozen_geometry_.yaw_rad;
        hook_lock_.locked_size = Eigen::Vector3f(
            cargo_frozen_geometry_.length_m,
            cargo_frozen_geometry_.width_m,
            cargo_frozen_geometry_.height_m);
        hook_lock_.stable_height = cargo_frozen_geometry_.height_m;
        hook_lock_.shape_height_valid = true;
    }
    std_msgs::String fusion_debug;
    std::ostringstream fusion_stream;
    const auto diagnostic_float = [](float value) {
        return std::isfinite(value) ? std::to_string(value)
                                    : std::string("null");
    };
    fusion_stream
        << "{\"cargo_lifecycle_id\":" << cargo_lifecycle_id_
        << ",\"cargo_track_segment_id\":" << cargo_track_segment_id_
        << ",\"lift_origin_state\":\""
        ...
        << "\",\"map_session_uuid\":\""
        << loaded_map_session_uuid_
        << "\",\"map_session_generation\":"
        << loaded_map_session_generation_
        << ",\"map_session_verified\":"
        << (verified_map_session_loaded_ ? "true" : "false") << "}";
    fusion_debug.data = fusion_stream.str();
...
void NdtSlamNode::runPendingCargoAvoidance(
    ...
    if (obstacle_cloud_base) {
        ...
        for (const pcl::PointXYZ& point : obstacle_cloud_base->points) {
            ...
            // This is an evaluator-local exterior view. It never mutates the
            // registration/map clouds and never grants formal cargo removal.
            if (containsPointInCargoObbBase(
                    xyz, live_input.footprint_base, 0.0F, 0.0F)) continue;
            if (pointToCargoObbDistance2D(
                    xyz.head<2>(), live_input.footprint_base) <= shell_m) {
    ...
    const CargoSafetyResult live_result =
        cargo_safety_evaluator_.evaluate(live_input);
    ...
    if (height_field && envelope.valid) {
        ...
        query.yaw_map_rad = static_cast<float>(
            std::atan2(rotation(1, 0), rotation(0, 0))) +
            envelope.yaw_base_rad;
        query.shell_m = shell_m;
        static_result = height_field->query(query);
        fusion_input.static_map.available = static_result.valid;
        fusion_input.static_map.coverage = static_result.coverage_ratio;
        if (static_result.valid && static_result.matched_cells > 0U &&
            std::isfinite(static_result.highest_z95_m) &&
            std::isfinite(static_result.nearest_horizontal_distance_m)) {
            ...
            fusion_input.static_map.hazard = clearance <
                    cargo_safety_evaluator_.config()
                        .minimum_vertical_clearance_m &&
                static_result.nearest_horizontal_distance_m <= shell_m;
            if (fusion_input.static_map.hazard) {
        ...
        fusion_input.static_map.reliable = static_result.valid &&
            static_result.bounded &&
            (fusion_input.static_map.hazard ||
             static_result.coverage_ratio >=
                 cargo_avoidance_fusion_config_
                     .minimum_live_coverage_for_clear);
        fusion_input.static_map.reason = static_result.reason;
    }
    ...
    fusion_input.static_session_manifest_valid =
        runtime_contract || verified_contract;
    fusion_input.static_session_hash_valid =
        fusion_input.static_session_manifest_valid;
    fusion_input.static_session_uuid_valid =
        fusion_input.static_session_manifest_valid;
    ...
    stream << "{\"cargo_lifecycle_id\":" << cargo_lifecycle_id_
           << ",\"cargo_track_segment_id\":" << cargo_track_segment_id_
           << ",\"lift_origin_state\":\""
           << cargoLiftEventStateName(cargo_lift_origin_result_.state)
           << "\",\"lift_origin_source\":"
           ...
           << ",\"map_live_conflict\":"
           << (fused.map_live_conflict ? "true" : "false")
           << ",\"static_height_query_reason\":\""
    ...
    const std::int32_t provisional_code = fused.official_valid &&
        ...
        ? fused.official_code : 0;
    std::uint32_t provisional_count = 0U;
    float provisional_distance =
        std::numeric_limits<float>::quiet_NaN();
    float provisional_top_map =
        std::numeric_limits<float>::quiet_NaN();
    float provisional_uncertainty =
        std::numeric_limits<float>::quiet_NaN();
    ...
    if (use_live_geometry) {
        ...
        provisional_distance = evidence.footprint_distance_m;
        provisional_uncertainty = evidence.obstacle_uncertainty_m;
        provisional_clearance = evidence.conservative_clearance_m;
        const Eigen::Vector3d obstacle_map = pose_map_base * Eigen::Vector3d(
            evidence.nearest_point_base.x,
            evidence.nearest_point_base.y,
            evidence.obstacle_top_z95_m);
        provisional_top_map = static_cast<float>(obstacle_map.z());
    } else if (fused.risk_static &&
               fusion_input.static_map.warning_code == provisional_code) {
        provisional_count = static_cast<std::uint32_t>(std::min<std::size_t>(
            static_result.matched_cells,
            std::numeric_limits<std::uint32_t>::max()));
        provisional_distance = fusion_input.static_map.distance_m;
        provisional_top_map = static_result.highest_z95_m;
    ...
    publishHookOnlySafetyStatus(
        ...
        false, provisional_code, provisional_count, provisional_distance,
    ...
    marker.ns = "pending_cargo_envelope";
    marker.id = 0;
    marker.type = visualization_msgs::Marker::CUBE;
    marker.action = envelope.valid
        ? visualization_msgs::Marker::ADD
        : visualization_msgs::Marker::DELETE;
    marker.pose.orientation.w = 1.0;
    if (envelope.valid) {
        marker.pose.position.x = envelope.center_base.x();
        marker.pose.position.y = envelope.center_base.y();
        marker.pose.position.z = envelope.center_base.z();
        const float half_yaw = 0.5F * envelope.yaw_base_rad;
        marker.pose.orientation.z = std::sin(half_yaw);
        marker.pose.orientation.w = std::cos(half_yaw);
        marker.scale.x = envelope.length_m;
        marker.scale.y = envelope.width_m;
        marker.scale.z = envelope.height_m;
        marker.color.r = 1.0F;
        marker.color.g = 0.55F;
        marker.color.b = 0.0F;
        marker.color.a = 0.28F;
    }
    cargo_pending_envelope_marker_pub_.publish(marker);
}
...
void NdtSlamNode::updateAndPublishCargoSafetyPipeline(
    ...
    updateCargoLiftAndGeometryFusion(
        hook, pose_map_base, stamp, active_track);
    cargo_fusion_track_active_ = active_track;
    if (!active_track) {
        const bool hook_loaded = hook.valid &&
            hook.state == lidar_slam2_msgs::HookLoadState::STATE_LOADED;
        if (hook_loaded && pending_cargo_envelope_.valid) {
            formal_cargo_removal_authorized_ = false;
    ...
    if (active_track) {
        ...
        observation.frozen_thickness_valid =
            ...
            frozen_thickness <=
                cargo_bottom_fusion_.config().maximum_prior_height;
        observation.frozen_thickness_m = frozen_thickness;
        observation.frozen_thickness_stamp_sec =
            hook_lock_.last_good_height_stamp.isZero()
                ? hook_lock_.live_pose.evidence_stamp_sec
                : hook_lock_.last_good_height_stamp.toSec();
        observation.frozen_thickness_confidence = std::clamp(
            hook_lock_.locked_shape.orientation_confidence,
            0.0F, 1.0F);
        const bool authorized_origin = cargo_lift_origin_result_.valid &&
            (cargo_lift_origin_result_.origin.source ==
        ...
        observation.map_static_height_m =
            cargo_lift_origin_result_.static_thickness_m;
        observation.map_diff_height_valid = authorized_origin &&
            cargo_lift_origin_result_.lift_confirmed &&
            std::isfinite(cargo_lift_origin_result_.revealed_thickness_m);
    ...
    if (rigid_geometry.valid) {
        ...
        last_cargo_bottom_result_.evidence_stamp_sec =
            formal_height.evidence_stamp_sec;
        last_cargo_bottom_result_.source = formal_height.source;
        last_cargo_bottom_result_.source_name =
            cargoBottomSourceName(formal_height.source);
        last_cargo_bottom_result_.reason = formal_height.reason;
        last_cargo_bottom_result_.uncertainty =
            formal_height.valid
                ? formal_height.uncertainty_m
                : cargo_bottom_fusion_.config().invalid_uncertainty;
        last_cargo_bottom_result_.confidence = formal_height.confidence;
    ...
    if (radial_safety_result.input_valid &&
        ...
        for (std::size_t evidence_index = 0U;
            ...
            if (std::isfinite(min_x) && std::isfinite(max_x) &&
                ...
                observation.occupied_cells = occupied_map_cells.size();
            }
            observation.occupied_map_cells.assign(
                occupied_map_cells.begin(), occupied_map_cells.end());
    ...
    cargo_obstacle_static_provenance_streak_ =
        obstacle_track_decision.selected_static_provenance_streak;
    cargo_obstacle_static_age_sec_ =
        obstacle_track_decision.selected_static_age_sec;
    cargo_obstacle_large_geometry_valid_ =
        obstacle_track_decision.selected_large_geometry_valid;
    cargo_obstacle_provenance_ =
        obstacle_track_decision.selected_provenance;
    cargo_obstacle_provenance_valid_ =
        obstacle_track_decision.selected_provenance_valid;
    cargo_obstacle_track_cell_overlap_ =
        obstacle_track_decision.selected_track_cell_overlap;
    cargo_obstacle_track_iou_ =
        obstacle_track_decision.selected_track_iou;
    cargo_obstacle_association_cost_ =
        obstacle_track_decision.selected_association_cost;
    ...
    // Fuse the bounded live ROI with the sparse static height field. Positive
    // hazards may come from either reliable source, but code 14 requires both
    // sources to be reliable. A live-clear/static-hazard conflict therefore
    // retains the static warning and is made explicit in the reason string.
    ...
    avoidance_input.live.hazard =
        avoidance_input.live.reliable &&
        (last_cargo_safety_result_.warning_code ==
             CargoSafetyEvaluator::kLevel1Code ||
         last_cargo_safety_result_.warning_code ==
             CargoSafetyEvaluator::kLevel2Code);
    avoidance_input.live.warning_code =
        last_cargo_safety_result_.warning_code;
    avoidance_input.live.coverage = coverage_ratio;
    if (last_cargo_safety_result_.has_cluster_evidence) {
        avoidance_input.live.distance_m =
            last_cargo_safety_result_.most_dangerous_cluster
                .footprint_distance_m;
        avoidance_input.live.clearance_m =
            last_cargo_safety_result_.most_dangerous_cluster
                .conservative_clearance_m;
    }

    StaticHeightQueryResult static_height_result;
    const auto static_height_field =
        std::atomic_load(&static_height_field_);
    const auto static_evidence_snapshot =
        static_obstacle_evidence_index_.snapshot();
    if (static_height_field && rigid_geometry.valid &&
        cargo_center_map.allFinite()) {
        StaticHeightQuery static_query;
        static_query.center_map = cargo_center_map;
        static_query.length_m = rigid_geometry.shape.length_m +
            2.0F * formal_use.horizontal_uncertainty_m;
        static_query.width_m = rigid_geometry.shape.width_m +
            2.0F * formal_use.horizontal_uncertainty_m;
        const Eigen::Matrix3d map_rotation =
            pose_map_base.so3().matrix();
        static_query.yaw_map_rad =
            static_cast<float>(std::atan2(
                map_rotation(1, 0), map_rotation(0, 0))) +
            rigid_geometry.shape.yaw_base_rad;
        ...
        avoidance_input.static_map.coverage =
            static_height_result.coverage_ratio;
        const bool static_has_hazard_candidate =
            avoidance_input.formal_cargo_bottom_valid &&
            static_height_result.valid &&
            static_height_result.matched_cells > 0U &&
            std::isfinite(static_height_result.highest_z95_m) &&
            std::isfinite(static_height_result
                              .nearest_horizontal_distance_m);
        if (static_has_hazard_candidate) {
            const float conservative_bottom =
                last_cargo_bottom_result_.geometry.bottom_z_map -
                last_cargo_bottom_result_.uncertainty -
                cargo_safety_evaluator_.config()
                    .cargo_bottom_extra_margin_m;
            const float clearance = conservative_bottom -
                static_height_result.highest_z95_m -
                static_height_result.highest_uncertainty_m;
            avoidance_input.static_map.distance_m =
                static_height_result.nearest_horizontal_distance_m;
            avoidance_input.static_map.clearance_m = clearance;
            avoidance_input.static_map.hazard =
                clearance < cargo_safety_evaluator_.config()
                                .minimum_vertical_clearance_m &&
                static_height_result.nearest_horizontal_distance_m <=
                    cargo_safety_evaluator_.config().level2_distance_m;
            if (avoidance_input.static_map.hazard) {
        ...
        avoidance_input.static_map.reliable =
            static_height_result.valid && static_height_result.bounded &&
            (avoidance_input.static_map.hazard ||
             static_height_result.coverage_ratio >=
                 cargo_avoidance_fusion_config_
                     .minimum_live_coverage_for_clear);
        avoidance_input.static_map.reason = static_height_result.reason;
    }
    const StaticEvidenceAuthority static_authority =
        static_evidence_snapshot
            ? static_evidence_snapshot->authority
            : StaticEvidenceAuthority::UNVERIFIED_LOADED_CLEAN;
    avoidance_input.static_authority = static_authority;
    const bool runtime_static_contract =
        static_authority == StaticEvidenceAuthority::RUNTIME_MATURE &&
        static_obstacle_evidence_index_.matureCellCount() > 0U;
    const bool verified_static_contract = verified_map_session_loaded_ &&
        (static_authority ==
             StaticEvidenceAuthority::OPERATOR_APPROVED_BASELINE ||
         runtime_static_contract);
    const bool static_contract =
        verified_static_contract || runtime_static_contract;
    avoidance_input.static_session_manifest_valid = static_contract;
    avoidance_input.static_session_hash_valid = static_contract;
    avoidance_input.static_session_uuid_valid = static_contract;
    ...
    if (fused_avoidance.official_valid &&
        (fused_avoidance.official_code ==
         ...
         fused_avoidance.official_code ==
             CargoSafetyEvaluator::kLevel2Code) &&
        fused_avoidance.risk_static) {
        last_cargo_safety_result_.input_valid = true;
        last_cargo_safety_result_.warning_valid = true;
        last_cargo_safety_result_.warning_code =
            static_cast<std::uint16_t>(fused_avoidance.official_code);
        last_cargo_safety_result_.fault = CargoSafetyFault::NONE;
        last_cargo_safety_result_.evidence_state =
            CargoSafetyEvidenceState::HAZARD_CONFIRMED;
        ...
        evidence.warning_code = last_cargo_safety_result_.warning_code;
        evidence.footprint_distance_m = fused_avoidance.distance_m;
        evidence.conservative_clearance_m = fused_avoidance.clearance_m;
        evidence.obstacle_uncertainty_m =
            static_height_result.highest_uncertainty_m;
        if (!static_height_result.matched_cell_keys.empty()) {
            const auto cell = unpackStaticEvidenceCell(
                static_height_result.matched_cell_keys.front());
            const float cell_size = static_height_field->config().cell_size_m;
            const Eigen::Vector3d nearest_map(
                (static_cast<double>(cell.first) + 0.5) * cell_size,
                (static_cast<double>(cell.second) + 0.5) * cell_size,
                static_height_result.highest_z95_m);
            const Eigen::Vector3d nearest_base =
                pose_map_base.inverse() * nearest_map;
            evidence.nearest_point_base.x =
                static_cast<float>(nearest_base.x());
            evidence.nearest_point_base.y =
                static_cast<float>(nearest_base.y());
            evidence.nearest_point_base.z =
                static_cast<float>(nearest_base.z());
            evidence.centroid_base = evidence.nearest_point_base;
            evidence.obstacle_top_z95_m =
                static_cast<float>(nearest_base.z());
        }
        last_cargo_safety_result_.has_cluster_evidence = true;
    ...
    const auto build_safety_message = [&](const CargoSafetyResult& result) {
        ...
        message.obstacle_track_id = static_cast<std::uint32_t>(
            std::min<std::uint64_t>(
                ...
                std::numeric_limits<std::uint32_t>::max()));
        message.obstacle_validated_streak = static_cast<std::uint32_t>(
            std::max(0, cargo_obstacle_track_confirm_count_));
        message.obstacle_static_provenance_streak =
            static_cast<std::uint32_t>(
                std::max(0, cargo_obstacle_static_provenance_streak_));
        message.obstacle_static_age_sec = static_cast<float>(
            std::max(0.0, cargo_obstacle_static_age_sec_));
        message.obstacle_large_geometry_valid =
            cargo_obstacle_large_geometry_valid_;
        message.obstacle_provenance_type =
            static_cast<std::uint8_t>(cargo_obstacle_provenance_);
        message.obstacle_provenance_valid =
            cargo_obstacle_provenance_valid_;
        message.obstacle_track_cell_overlap =
            cargo_obstacle_track_cell_overlap_;
        message.obstacle_track_iou = cargo_obstacle_track_iou_;
        message.obstacle_association_cost =
            cargo_obstacle_association_cost_;
        message.obstacle_association_reset_reason =
            cargo_obstacle_association_reset_reason_;
    ...
    const MapCommitJob& job)
{
    ...
    *commit_cloud_base += *objects_after_human_base;
    ...
    // 只用过滤后的 objects_commit_map，每个 BEV cell 只加一次
    {
        ...
        StaticEvidenceCellKeySet observed_free_static_cells;
        const float static_cell_size =
            static_obstacle_evidence_index_.config().cell_size_m;
        ...
        // A ground return in a previously occupied XY cell is explicit free
        // evidence: the beam reached the support surface. Cells with no
        // current return remain NOT_IN_VIEW and never break a streak.
        for (const auto& p : ground_transformed.points) {
            if (!std::isfinite(p.x) || !std::isfinite(p.y) ||
                !std::isfinite(p.z) || std::abs(p.x) > max_map_size_ ||
                std::abs(p.y) > max_map_size_ ||
                std::abs(p.z) > max_map_size_) {
                continue;
            }
            const double static_x = std::floor(p.x / static_cell_size);
            const double static_y = std::floor(p.y / static_cell_size);
            if (static_x < std::numeric_limits<std::int32_t>::lowest() ||
                static_x > std::numeric_limits<std::int32_t>::max() ||
                static_y < std::numeric_limits<std::int32_t>::lowest() ||
                static_y > std::numeric_limits<std::int32_t>::max()) {
                continue;
            ...
            const std::int64_t key = packStaticEvidenceCell(
                static_cast<std::int32_t>(static_x),
                static_cast<std::int32_t>(static_y));
            if (observed_static_cells.find(key) ==
                observed_static_cells.end()) {
                observed_free_static_cells.insert(key);
            }
        }
        ...
        if (job.allow_persistent_map_commit) {
            // Raw MapCommit frames collect actual occupied ROI evidence, but
            // do not advance temporal maturity. One completed clean build is
            // the sole observation sequence owner; cells absent from this
            // accumulator are NOT_IN_VIEW. A support-surface return is the
            // only observation that creates explicit free evidence.
            std::lock_guard<std::mutex> observation_lock(
                static_observation_mutex_);
            for (const std::int64_t key : observed_free_static_cells) {
                pending_static_observable_cells_.insert(key);
                pending_static_free_cells_.insert(key);
                pending_static_observed_cells_.erase(key);
            }
            for (const auto& item : observed_static_cells) {
                pending_static_observable_cells_.insert(item.first);
                pending_static_free_cells_.erase(item.first);
                auto inserted = pending_static_observed_cells_.emplace(
                    item.first, item.second);
                if (!inserted.second) {
                    inserted.first->second.min_z = std::min(
...
void NdtSlamNode::logCargoHealthPeriodic() {
    ...
    rec.obstacle_static_provenance_streak =
        cargo_obstacle_static_provenance_streak_;
    rec.obstacle_static_age_sec = cargo_obstacle_static_age_sec_;
    rec.obstacle_large_geometry_valid =
        cargo_obstacle_large_geometry_valid_;
    rec.obstacle_provenance_type =
        externalProvenanceName(cargo_obstacle_provenance_);
    rec.obstacle_provenance_valid =
        cargo_obstacle_provenance_valid_;
    rec.obstacle_track_cell_overlap =
        cargo_obstacle_track_cell_overlap_;
    rec.obstacle_track_iou = cargo_obstacle_track_iou_;
    ...
    switch (hook_lock_.state) {
        ...
        case HookCargoLockState::CLEAR_WAIT_REARM:
            ...
            break;
        case HookCargoLockState::LOADED_REACQUIRE:
            rec.lock_state = "LOADED_REACQUIRE";
            break;
    }
