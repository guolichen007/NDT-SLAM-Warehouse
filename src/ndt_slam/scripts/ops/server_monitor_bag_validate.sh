...
for name in ("persistent_map_manifest.json", "static_evidence_manifest.json"):
    ...
    if os.path.exists(p):
        with open(p) as f:
            d = json.load(f)
        if d.get("map_frame_uuid"):
            # schema2 persistent map: path-independent map_frame_uuid must stay
            # byte-identical across sandbox copies; never restamp semantic id.
            print(f"  sandbox {name}: map_frame_uuid preserved "
                  f"({d['map_frame_uuid']})")
            continue
