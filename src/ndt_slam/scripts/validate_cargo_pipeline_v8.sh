...
LOG_FILE="${1:-/tmp/cargo_v8_real.log}"

echo "========== Cargo Pipeline Validation v8 =========="
...
# XmlRpc 错误可能来自 rosbag 或其他节点，只检查我们节点相关的
if grep "XmlRpcClient::writeRequest" "${LOG_FILE}" | grep -q "ndt_slam\|cargo_forbidden"; then
  fail "XmlRpcClient write error from our nodes"
fi

if grep -q "base_link_to_rslidar.*process has finished" "${LOG_FILE}"; then
  fail "base_link_to_rslidar TF node was killed"
fi

pass_msg "launch clean (no duplicate nodes)"

# ========== 2. PoseFreeze 检查 ==========

if grep -q "\[PoseFreeze\].*mode=STATIONARY_ANCHOR" "${LOG_FILE}"; then
  FREEZE_COUNT=$(grep -c "\[PoseFreeze\].*mode=STATIONARY_ANCHOR" "${LOG_FILE}" || true)
  pass_msg "PoseFreeze STATIONARY_ANCHOR observed: ${FREEZE_COUNT} times"
else
  warn_msg "no PoseFreeze observed (crane may not have been stationary)"
fi

# ========== 3. PipelineSummary 检查 ==========
...
if grep -q "\[MotionGate\] Crane stopped" "${LOG_FILE}"; then
  pass_msg "MotionGate Crane stopped observed"
else
  warn_msg "no Crane stopped observed"
fi
...
# ========== 17. CargoMarkerGate 检查 ==========

if grep -q "\[CargoMarkerGate\]" "${LOG_FILE}"; then
  pass_msg "CargoMarkerGate exists"
else
  warn_msg "missing [CargoMarkerGate]"
fi

echo ""
echo "========== [VALIDATION] PASS: cargo pipeline v8 checks passed =========="
