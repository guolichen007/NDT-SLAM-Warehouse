    ...
    with open(filename, 'r') as f:
        ...
        for line in f:
            ...
            if len(parts) >= 7:
                ...
                vertical_span = float(parts[5])
                grid[(cx, cy)] = {
                    'ground_z': ground_z,
                    'vertical_span': vertical_span
                }

                # 从 world_x 推算 grid_size
                if grid_size is None and cx > 0:
                    wx = float(parts[2])
                    # 简化处理，使用配置中的 grid_size

    print(f"    加载了 {len(grid)} 个网格")
    return grid

def filter_objects(points, ground_model, xy_min, grid_size, config):
    """过滤物体点"""
    min_height = config['min_height_above_ground']
    max_height = config['max_height_above_ground']

    print(f"  过滤物体点 (height=[{min_height}, {max_height}]m)")
    ...
    for p in points:
        ...
        ground_z = ground_model[ci]['ground_z']
        local_height = p[2] - ground_z

        if min_height <= local_height <= max_height:
            kept_points.append(p)
        else:
            if local_height < min_height:
                removed_reasons['too_low'] += 1
            else:
                removed_reasons['too_high'] += 1
...
def radius_filter(points, radius=0.25, min_neighbors=3):
    """半径离群过滤"""
...
def main():
    parser = argparse.ArgumentParser(description='物体清理')
    ...
    oc_config = config.get('objects_clean', {})
    ...
    # 半径离群过滤
    if oc_config.get('remove_small_clusters', True):
        object_points = radius_filter(
            object_points,
            radius=oc_config.get('radius_filter_radius', 0.25),
            min_neighbors=oc_config.get('radius_filter_min_neighbors', 3)
        )

    # 体素下采样
    object_points = voxel_downsample(object_points, oc_config.get('voxel_size', 0.08))

    # 保存结果
    os.makedirs(output_dir, exist_ok=True)

    objects_clean_file = os.path.join(output_dir, 'objects_clean.pcd')
    save_pcd(objects_clean_file, object_points)
    print(f"\n  objects_clean.pcd 已保存: {len(object_points):,} 点")
    ...
    report = {
        ...
        'output_points': int(len(object_points)),
        'config': oc_config,
    }

    report_file = os.path.join(output_dir, 'objects_clean_report.json')
