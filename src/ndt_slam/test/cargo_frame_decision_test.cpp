...
TEST(CargoFrameDecisionTest, ReviewAlsoRequiresAtomicHazardIdentity) {
  CargoFrameDecision decision = warningDecision();
  decision.warning_code = 29;
  decision.authoritative_warning_code = 29;
  EXPECT_TRUE(commitCargoFrameDecision(decision).authorized);

  decision.obstacle_track_id = 0U;
  const auto rejected = commitCargoFrameDecision(decision);
  EXPECT_FALSE(rejected.authorized);
  EXPECT_EQ(rejected.status_code, 35);
}
