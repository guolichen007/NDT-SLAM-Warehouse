...
扇区边界计入前方，侧后方普通障碍不会授权 17/18；距离货物不超过 0.30m 的实时接触级候选仍保留全方向检测，但只输出 Code 29。速度无效或低于运动门限时不复用旧方向，而是用径向距离继续维护 Track；已有远场历史或独立成熟静态来源的危险仍可输出 17/18，未确认候选不得借此升级。
...
Code 17。障碍查询从 8m 开始建 Track，5-8m 只积累身份、方向、接近趋势和静态来源，
不输出 17/18。实时新障碍仍必须先有远场接近历史；缺少该历史却突然进入 3m 时
...
实时点簇和静态高度场可以同时给出候选，但最终状态只选择一条权威危险记录。Code、
OBB 距离、垂直净空、障碍 Track 和来源必须来自同一候选；先按 17 高于 18 选择，同级
再选距离更近者，禁止把静态 Code 与实时距离或净空混合。成熟静态 Code 17 的优先级
高于同帧实时突发近场的 Code 29。

Code 29 是有界人工复核事件，不是持续告警状态。相同货物生命周期、货物 Track、障碍
Track、位姿连续性和地图代次下，需要连续 2 个严格前进时间戳的新帧才进入；最多保持
1.5 秒，随后降级为 34，并在 2 秒冷却内禁止同一事件重新进入。标准 17/18、障碍消失、
生命周期变化、时间回退或位姿连续性变化都会结束事件。heartbeat 重发和重复时间戳
不会累计确认或周期性重复触发截图。

### Hook 状态兼容与新鲜度

Hook 对外状态严格保持消息 schema v1 的 `UNKNOWN/INHIBIT/EMPTY/LOADED`，不发布私有
枚举值。输入短暂中断时可在 5 秒内保持最近的 EMPTY/LOADED，但同时设置
`fresh=false` 与 `held_stale=true`；重复源时间戳既不刷新证据，也不能解除 held 状态。
只有严格前进的有效样本才能恢复新鲜度，超过长期失效门限后回到 UNKNOWN。
