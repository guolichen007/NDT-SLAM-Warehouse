...
struct CargoObstacleTrackerConfig {
  ...
  // A formal 17/18 warning must belong to the same physical track that was
  // confirmed in the true 5-8 m acquisition shell.  The frame/duration
  // thresholds are tunable evidence strength, not fixed business constants.
...
struct CargoObstacleObservation {
  ...
  // Combined cargo/obstacle/localization XY uncertainty.  Far-history
  // evidence is accepted only when distance - uncertainty remains > 5 m.
  ...
  // Runtime/legacy static evidence is deliberately not certification.  Only
  // an immutable, identity-checked baseline may set this bit.
...
// Associates warning clusters and directional/radial 5-8 m acquisition
