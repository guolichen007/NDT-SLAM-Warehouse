...
struct NdtFrameRecord {
  ...
  std::string local_map_pose_authority = "INVALID";
  uint64_t local_map_consecutive_prediction_only_frames = 0;
  double local_map_prediction_only_duration_sec = 0.0;
  uint64_t local_map_frames_since_last_trusted_ndt = 0;
  double local_map_time_since_last_trusted_ndt_sec = 0.0;
  double local_map_distance_since_last_trusted_ndt_m = 0.0;
  double local_map_yaw_since_last_trusted_ndt_rad = 0.0;
  bool local_map_update_eligible = false;
  bool local_map_update_due = false;
  bool local_map_update_committed = false;
  std::string local_map_update_mode = "NONE";
  std::string local_map_health_state = "QUARANTINED";
  uint64_t local_map_version = 0;
  uint64_t local_map_point_count = 0;
  uint64_t local_map_updates_from_measured_pose = 0;
  uint64_t local_map_updates_from_predicted_pose = 0;
  uint64_t local_map_motion_escape_refresh_count = 0;
...
struct CargoFrameRecord {
  ...
  bool obstacle_source_validated = false;
  std::string obstacle_source_validation_reason;
  double obstacle_inside_xy_ratio = 0.0;
  double obstacle_identity_match_ratio = 0.0;
  double obstacle_surface_band_ratio = 0.0;
  double obstacle_motion_match_score = 0.0;
  double obstacle_validation_shell_m = 0.0;
  std::uint64_t static_index_epoch = 0U;
  std::uint64_t static_expected_epoch = 0U;
  std::uint64_t static_index_revision = 0U;
  std::uint64_t static_index_latest_sequence = 0U;
  std::uint64_t static_cargo_track_start_sequence = 0U;
  std::size_t static_index_cell_count = 0U;
  std::size_t static_query_cell_count = 0U;
  std::size_t static_matched_cell_count = 0U;
  std::size_t static_spatial_cell_count = 0U;
  double static_matched_ratio = 0.0;
  double static_matched_iou = 0.0;
  double static_height_overlap = 0.0;
  std::uint32_t static_min_observation_count = 0U;
  double static_min_stable_age_sec = 0.0;
  bool static_authorized = false;
  std::string static_query_reason;
  std::size_t obstacle_point_count = 0U;
  std::size_t obstacle_xy_occupied_cells = 0U;
  double obstacle_xy_area_m2 = 0.0;
  double obstacle_long_side_m = 0.0;
  double obstacle_height_span_m = 0.0;
  std::size_t obstacle_required_point_count = 0U;
  double obstacle_required_xy_area_m2 = 0.0;
  double obstacle_required_long_side_m = 0.0;
  double obstacle_required_height_span_m = 0.0;
  std::size_t obstacle_required_occupied_cells = 0U;
  std::uint64_t obstacle_track_created_count = 0U;
  std::uint64_t obstacle_track_reset_count = 0U;
  std::uint64_t static_clean_build_started = 0U;
  std::uint64_t static_clean_build_applied = 0U;
  std::uint64_t static_clean_build_snapshot_only = 0U;
  std::uint64_t static_clean_build_discarded = 0U;
  std::uint64_t static_clean_confirmed_cells = 0U;
  std::uint64_t static_clean_invalidated_cells = 0U;
  std::uint64_t static_clean_snapshot_cells = 0U;
...
private:
  ...
  std::ofstream static_evidence_csv_;
