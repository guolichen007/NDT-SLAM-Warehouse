...
class SummarizeSafetyStatusTest(unittest.TestCase):
    def test_ValidFarToNearAndTwoFrameReviewPass(self):
        rows = [
    ...
    def test_ClearanceGateAndDistanceBandsAreChecked(self):
        rows = [
            {"stamp": 1.0, "requested_alarm_code": 18,
             "nearest_cluster_distance": 2.0,
             "conservative_clearance_m": 0.8,
             "obstacle_track_id": 9},
        ]
        violations = SUMMARY.validate_avoidance_rows(rows)
        self.assertTrue(any("clearance_gate" in value for value in violations))
        self.assertTrue(any("code18_distance" in value for value in violations))

    def test_Code29RequiresTwoDistinctSourceStamps(self):
        rows = [
            {"stamp": 3.0, "requested_alarm_code": 29,
             "nearest_cluster_distance": 4.0,
             "conservative_clearance_m": 0.5,
             "obstacle_track_id": 10},
        ]
        violations = SUMMARY.validate_avoidance_rows(rows)
        self.assertTrue(any("code29_distinct_frames" in value
                            for value in violations))

    def test_CsvLoaderRejectsMissingContractColumns(self):
