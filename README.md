...
- **定位自动恢复**：旧地图启动先校验并进入 Code 31 隔离，优先使用 `objects_clean` 做有界全图搜索；候选需在最近 8 帧内至少 6 帧严格合格，且不能连续失败超过 2 帧。通过后由连续坏帧重定位策略处理运行期波动，单帧 prediction-only 不撤销已验证位置。健康运行期间另以不可变的持久 registration 快照做只读全局一致性复核，不能用正在更新的地图自证正确。响应正常但未恢复时等待新静止周期，只有进程级故障才由前台 supervisor 有预算地重启整栈。
...
- **障碍物追踪与安全码**：Code 14（CLEAR）/ 17（≤3m）/ 18（3-5m）/ 29（异常待复核）/ 30-35（故障）。障碍从 5-8m 开始建 Track，正式 17/18 只由运行方向前方 ±45° 内的真实空间碰撞风险产生。实时新障碍必须先形成远场接近历史再进入 17；绑定地图身份且独立成熟的静态障碍已有跨帧物理历史，可在首次进入 3m 时直接输出 17。0.30m 全方向接触保护和缺少历史的实时突发近场只进入有界 Code 29 事件。
...
| 29 | ANOMALY_REVIEW — 接触级或突发近场异常，需保存现场人工复核 | 连续 2 个新帧进入、最长 1.5 秒、同事件冷却 2 秒 |
| 30 | 系统未就绪 / 时间轴回退 | 故障 |
| 31 | 定位无效 | 故障 |
...
起吊前基线在 `EMPTY + 定位可靠` 时从同代静态高度场建立；普通组件仍要求静止，已独立成熟且早于本次货物生命周期的静态组件允许在移动中连续复核。它记录顶面、支撑面、
