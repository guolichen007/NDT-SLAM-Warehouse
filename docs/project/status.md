...
当前主线状态以本文件所在提交为准。最近运行时代码修复：`f57d68aee88d6d9a54babaf12046c04183b84863`。

现场验证基线：`8d7d7eed0548321bf0646232f374fe95a29990dd`（`validation-obstacle-avoidance-20260728`）

状态矩阵将“当前实现”和“历史现场证据”分开。`f57d68a` 及其父提交已做 Windows
静态检查，但没有在 Windows 上冒充 ROS/PCL/Sophus 编译；Ubuntu、Bag 和现场列只有
取得对应提交的实际证据后才会更新。
...
| NDT fitness 自适应熔断 | 已实现（`f57d68a`） | 通过 | `NOT_RUN` | `NOT_RUN` | 无当前实现证据 |
| 重定位结果多帧确认 | 已实现（`f57d68a`） | 通过 | `NOT_RUN` | `NOT_RUN` | 无当前实现证据 |
